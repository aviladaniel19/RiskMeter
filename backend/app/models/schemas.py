...
# ═══════════════════════════════════════════════════════
# ENDPOINT: /renta-fija/curva (GET)
# ═══════════════════════════════════════════════════════

class PuntoCurvaReal(BaseModel):
    serie: str
    nombre: str
    maduracion: float
    tasa: float
...
class ParametrosNS(BaseModel):
    beta0: float
    beta1: float
    beta2: float
    tau: float

class CurvaTeorica(BaseModel):
    maduraciones: list[float]
    tasas: list[float]

class NSResultado(BaseModel):
    exito: bool
    parametros: ParametrosNS | None = None
    curva_teorica: CurvaTeorica | None = None
    error: str | None = None

class CurvaRendimientosResponse(BaseModel):
    """Respuesta del endpoint GET /renta-fija/curva."""
    fecha_datos: str
    puntos_reales: list[PuntoCurvaReal]
    nelson_siegel: NSResultado


# ═══════════════════════════════════════════════════════
# ENDPOINT: /derivados/black-scholes (POST)
# ═══════════════════════════════════════════════════════

class BlackScholesRequest(BaseModel):
    """Parámetros para la valoración de la opción."""
    S: float = Field(..., description="Precio actual del subyacente")
    K: float = Field(..., description="Precio de ejercicio (Strike)")
    T: float = Field(..., description="Tiempo al vencimiento en años (ej. 0.5 = 6 meses)")
    r: float = Field(..., description="Tasa libre de riesgo (en decimal, ej. 0.05 = 5%)")
    sigma: float = Field(..., description="Volatilidad anualizada (en decimal, ej. 0.20 = 20%)")
    tipo: str = Field(default="call", description="'call' o 'put'")

class GriegasResponse(BaseModel):
    delta: float
    gamma: float
    vega: float
    theta: float
...
class ValoracionOpcion(BaseModel):
    tipo: str
    precio: float
    griegas: GriegasResponse

class SuperficieResponse(BaseModel):
    x_subyacente: list[float]
    y_tiempo: list[float]
    z_precios: list[list[float]]

class BlackScholesResponse(BaseModel):
    """Respuesta del endpoint POST /derivados/black-scholes."""
    parametros: BlackScholesRequest
    valoracion: ValoracionOpcion
    superficie: SuperficieResponse


# ═══════════════════════════════════════════════════════
# ENDPOINT: /predict/{ticker} (GET)
# ═══════════════════════════════════════════════════════

class PredictionResponse(BaseModel):
    """Respuesta del endpoint GET /predict/{ticker}."""
    ticker: str
    señal: str = Field(..., alias="señal")
    confianza: float
    probabilidades: dict[str, float]
    importancia_variables: dict[str, float] = Field(..., alias="importancia_variables")
    modelo: str
    fecha_ejecucion: str
